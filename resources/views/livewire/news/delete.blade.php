<div>
    <x-secondary-button wire:click="openModalToDeleteArticle"><i class="fa-solid fa-trash"></i></x-secondary-button>

    <x-custom-modal wire:model="openModal" maxWidth="lg">
        <x-slot:title>
            <div class="text-2xl mb-4 flex justify-center items-center">
                <span>{{ __('Delete Article') }}</span>
            </div>
        </x-slot:title>
        <x-slot:content>
            <form wire:submit.prevent='delete' class="space-y-4" id="deleteArticleForm">
                @csrf
